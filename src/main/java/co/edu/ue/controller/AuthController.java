...
public class AuthController {
	...
	@Operation(summary = "Inicio de sesión", description = "Permite a los usuarios autenticarse proporcionando su correo electrónico y contraseña.", tags = {
			"Usuarios" })
	public ResponseEntity<?> login(@RequestBody LogInRequest logInInfo) throws Exception {
		...
		try {
			...
			String encryptedRefreshToken = TokenEncryptionUtil.encrypt(refreshToken);

			ResponseCookie refreshTokenCookie = ResponseCookie.from("refreshToken", encryptedRefreshToken)
					.httpOnly(true)
					.secure(true)
					.path("/")
					.maxAge(7 * 24 * 60 * 60)
	...
	@PostMapping("/refresh")
	@Operation(summary = "Refrescar Token de acceso", description = "Permite al servicio del cliente actualizar el token al haber expirado", tags = {
			"Usuarios" })
	public ResponseEntity<?> refreshAccessToken(@CookieValue @Parameter(hidden = true) String refreshToken)
			throws Exception {

		if (refreshToken == null || refreshToken.isEmpty()) {
			return new ResponseEntity<>(Map.of("Status", false, "Data", "Refresh token no proporcionado"),
					HttpStatus.UNAUTHORIZED);
		}

		try {
			String decryptedRefreshToken = TokenEncryptionUtil.decrypt(refreshToken);
			...
			if (user == null) {
				return new ResponseEntity<>(Map.of("Status", false, "Data", "Usuario no encontrado"),
						HttpStatus.UNAUTHORIZED);
			}
